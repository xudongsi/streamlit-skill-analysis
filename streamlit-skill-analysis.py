...
def load_sheets(file) -> Tuple[List[str], dict]:
    ...
    try:
        ...
        st.sidebar.error(f"[表情] 读取Excel文件失败：{e}")
        # 降级尝试xlrd引擎（兼容旧格式）
        try:
            xpd = pd.ExcelFile(file, engine="xlrd")
        except Exception as e2:
            st.sidebar.error(f"[表情] xlrd引擎也读取失败：{e2}")
            return [], {}
...
def delete_sheet_optimized(file_path, sheet_name):
    if not os.path.exists(file_path):
        return False, "[表情] 文件不存在"

    if is_file_locked(file_path):
        return False, "[表情] 文件被占用（可能Excel已打开），请关闭Excel后重试"
    ...
    try:
        ...
        if sheet_name not in wb.sheetnames:
            wb.close()
            return False, "[表情] 工作表不存在"

        wb.remove(wb[sheet_name])
        wb.save(file_path)
        wb.close()

        return True, f"[表情] 成功删除工作表: {sheet_name}"
    except PermissionError:
        return False, "[表情] 权限不足，无法删除工作表（请检查文件是否只读）"
    except Exception as e:
        return False, f"[表情] 删除失败: {str(e)}"
...
if not os.path.exists(SAVE_FILE):
    try:
        ...
        sheets, sheet_frames = load_sheets(SAVE_FILE)
        st.sidebar.success(f"[表情] 已创建初始文件 {SAVE_FILE}")
    except Exception as e:
        st.sidebar.error(f"[表情] 创建初始文件失败：{e}")
elif not sheets:
    st.sidebar.warning("[表情] 文件存在但无有效工作表，已创建示例数据")
    sheet_frames = {
    ...
    st.sidebar.success(f"[表情] 已加载库文件 {SAVE_FILE}（共{len(sheets)}个工作表）")
...
if repaired_frames:
    try:
        ...
        st.sidebar.error(f"[表情] 修复数量总和失败：{e}")
...
if sheets:
    ...
    if len(sheets) == 1:
        ...
        if not st.session_state.delete_confirm:
            ...
            st.sidebar.warning(f"[表情] 确认删除【{sheet_to_delete}】？此操作不可恢复！")
...
if not time_candidates:
    st.warning(f"[表情] 暂无符合条件的数据，请先创建月份或季度。")
    time_choice = []
else:
...
if view == "编辑数据":
    if not time_choice:
        ...
        st.warning("[表情] 编辑数据时仅支持选择单个时间点，请重新选择！")
    else:
        show_cards(df)
        ...
        try:
            ...
            if st.button("[表情] 保存修改到库里"):
                try:
                    ...
                    st.cache_data.clear()
                    sheets, sheet_frames = load_sheets(SAVE_FILE)
                    st.success(f"[表情] 修改已保存到 {SAVE_FILE} ({sheet_name})，仅更新选中分组数据")
                except Exception as e:
                    st.error(f"保存失败：{e}")
        except Exception as e:
            st.error(f"[表情] 加载编辑数据失败：{e}")

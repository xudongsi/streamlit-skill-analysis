...
if time_choice:
    ...
    if dfs:
        ...
        all_groups = combined_df["分组"].dropna().unique().tolist()
    ...
    if len(time_choice) > 1 and "时间点" in df0.columns:
        ...
        if len(time_points) == 1:
            ...
            fig.update_layout(barmode="stack", template="plotly_dark",
                              xaxis_title="任务", yaxis_title="完成值",
            ...
            for i, tp in enumerate(time_points, 1):
                ...
                for j, emp in enumerate(df_pivot.columns):
                    ...
                    fig.add_trace(
                        go.Bar(x=df_pivot.index, y=df_pivot[emp], name=emp,
                               marker_color=colors[color_idx], showlegend=(i == 1)),
                        row=i, col=1
                    )

            fig.update_layout(barmode="stack", template="plotly_dark", height=400 * len(time_points))
...
def chart_heat(df0):
    ...
    if len(time_choice) > 1 and "时间点" in df0.columns:
        ...
        for tp in time_points:
            df_tp = df0[df0["时间点"] == tp]
            tasks = df_tp["明细"].unique().tolist()
            emps = df_tp["员工"].unique().tolist()
            data = []
...
def show_cards(df0):
    ...
    c4.markdown(
        ...
        unsafe_allow_html=True)
    c5.markdown(
        f"<div class='metric-card'><div class='metric-label'>选择的时间点</div><div style='font-size:14px;margin-top:10px;color:#4cc9f0'>{time_points_display}</div></div>",
        unsafe_allow_html=True)
        ...
        if st.button("💾 保存修改到库里"):
            try:
                if len(time_choice) == 1:
                    ...
                    st.success(f"✅ 修改已保存到 {len(time_choice)} 个时间点")
